...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, n_heads, d_ff, dropout_p=0.1):
        ...
        self.attention1 = MultiHeadAttention(d_model, n_heads, dropout_p)
        self.attention2 = MultiHeadAttention(d_model, n_heads, dropout_p)
        self.feed_forward = FeedForward(d_model, d_ff, dropout_p )
    ...
    # tgt_mask = look ahead, src_mask = padding mask
    def forward(self, x, encoder_output, src_mask=None, tgt_mask=None):

        attn_output1, _ = self.attention1(x ,x ,x, tgt_mask)
        x = self.norm1(x + self.dropout(attn_output1))

        attn_output2, _ = self.attention2(x, encoder_output, encoder_output, src_mask)
